...
> 文档状态：`0.2.0-rc.3` 基线已冻结；`0.3.0-rc.1` 进入发布候选收口，公众号自动草稿主链路已通过真实账号验收
> 软件作者：Tbin · 联系邮箱：niuboy5188@gmail.com
> 当前版本：1.5-draft
> 下一里程碑：完成 `0.3.0-rc.1` 发布候选收口；本公益免费项目暂不采购商业代码签名证书，RC 可按未签名方案发布，签名流水线保留为未来可选能力。多账号、群发和定时群发暂不开发。
...
- 当前源码版本已推进到 `0.3.0-rc.1`，安装版、便携版和 SHA-256 清单已生成；42 个测试文件、224 项测试、严格类型检查、便携隔离启动、七组生产冒烟及安装—启动—卸载门禁通过。公益发布允许保持 `NotSigned`，不把商业证书作为 RC 阻断条件。多账号、群发和定时群发不进入当前计划。
...
当前所见即所得复制只写 `text/plain`，而公众号方案 B 的 HTML 剪贴板属于独立输出链路。本项用于补齐与 Typora、Word、浏览器和其他富文本编辑器之间的通用交换能力，不改变 canonical `editorText` 仍是唯一保存来源的约束。
...
- 每次智能粘贴只生成一个可撤销事务，绑定当前 `documentId`、`sourceHash` 和插入范围；解析失败、快照过期或安全审计不通过时不得改动 `editorText`，并提供“以纯文本粘贴/以 Markdown 粘贴”显式降级入口。
...
- 导出菜单已提供“主题与手机预览”，可即时切换三套主题和 320px / 375px / 414px 视口；首个 H1 按真实公众号投影从正文预览移除。三个宽度在后台并行审计，用户不必逐档手动触发；每档按钮独立显示通过、复核或警告状态。
...
- 模态窗口显示期间必须暂停背景文档快捷键。公众号主题预览打开时保存先前焦点，把焦点放入弹窗；Tab/Shift+Tab 形成焦点圈，Escape 关闭，关闭后将焦点返回“导出”入口。
...
- 当前源码版本为 `0.3.0-rc.1`。真实账号草稿验收、42/224 自动测试、便携版隔离启动、七组生产冒烟及安装—启动—卸载门禁均已通过，且不得覆盖历史 `0.2.0-rc.*` 产物。
...
- fantastic-editor 当前定位为公益免费项目，暂不采购商业代码签名证书；商业证书费用不纳入 `0.3.0-rc.1` 的完成条件。
