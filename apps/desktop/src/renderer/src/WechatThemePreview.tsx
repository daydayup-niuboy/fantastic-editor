...
import { WECHAT_THEME_OPTIONS, applyWechatThemeToFragment, resolveWechatTheme, type WechatThemeId } from "@fantastic-editor/shared";
...
export function WechatThemePreview({ html, themeId, fontFamily, onThemeChange, onClose }: WechatThemePreviewProps) {
  ...
  const selectedTheme = resolveWechatTheme(themeId);
  const themedHtml = useMemo(() => `<section style="${selectedTheme.wrapperStyle}">${applyWechatThemeToFragment(withoutLeadingPreviewTitle(html), themeId)}</section>`, [html, selectedTheme.wrapperStyle, themeId]);
  ...
  return (
    <div className="wechat-preview-overlay" role="dialog" aria-modal="true" aria-labelledby="wechat-preview-title" aria-describedby="wechat-preview-description" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="wechat-preview-dialog" ref={dialogRef}>
        <header>
        ...
        <div className="wechat-preview-controls">
          <label>主题<select value={themeId} onChange={(event) => onThemeChange(event.target.value as WechatThemeId)}>{WECHAT_THEME_OPTIONS.map((theme) => <option key={theme.id} value={theme.id}>{theme.name}</option>)}</select></label>
        ...
        <div className="wechat-audit-probes" aria-hidden="true">{MOBILE_WIDTHS.map((width) => <WechatAuditProbe key={width} width={width} html={themedHtml} fontFamily={fontFamily} onResult={handleAuditResult} />)}</div>
      </section>
    </div>
  );
}
