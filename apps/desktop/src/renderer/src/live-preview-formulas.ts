...
export function formulaSnapshotFromHtml(source: string, html: string): FormulaSnapshot {
  ...
  return { source, formulas: [...template.content.querySelectorAll('.preview-formula-inline, .preview-formula-block')]
...
class FormulaWidget extends WidgetType {
  constructor(readonly formula: Formula, readonly source: string) { super(); }
  eq(other: FormulaWidget): boolean { return this.source === other.source && JSON.stringify(this.formula) === JSON.stringify(other.formula); }
...
export function formulaDecorations(state: EditorState, snapshot: FormulaSnapshot | null): DecorationSet {
  ...
  let previousEnd = -1;
  return Decoration.set(snapshot.formulas.flatMap(formula => {
    ...
    return [Decoration.replace({ widget: new FormulaWidget(formula, snapshot.source), block: formula.block }).range(from, to)];
  }), true);
}
