...
编辑区可在“源代码”和“所见即所得”之间切换，选择会保存在本机。所见即所得模式复用预览主题和字体：标题与普通段落可直接编辑，并支持粗体、斜体、删除线、链接及标题级别；图片、公式、Mermaid、列表、表格、引用和代码块点击后通过精确绑定原文范围的源码卡片修改。两种模式共用 CodeMirror 的撤销/重做历史，保存和全部导出始终读取同一份 Markdown 原文。
...
当前基线：29 个测试文件、151 项测试通过；真实 Electron UI 冒烟已验证主界面、文档选项卡、新建文档、资源管理器、可调分栏、无横向溢出、拖拽覆盖层、主题切换、同步滚动、源代码/所见即所得切换、可视段落写回、共享撤销/重做和 Mermaid 源码卡片写回；公式 PNG、DOCX Utility Process、Chromium PDF 和系统 HTML 剪贴板生产冒烟均通过。
...
- 所见即所得首轮直接编辑标题和普通段落；复杂块使用源码卡片。所有写回都校验原文快照和 SourceRange，旧快照会被拒绝，不会整篇反序列化可编辑 HTML。
