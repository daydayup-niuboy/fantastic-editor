...
fantastic-editor 是面向 Windows 的本地优先 Markdown 编辑器，可打开单文件或文件夹工作区，编辑并预览包含本地图片与 KaTeX 公式的 Markdown，导出 PDF、Word、单文件离线 HTML，并支持微信公众号方案 B 兼容复制和 `0.3.0-rc.1` 的批量草稿同步。
...
公众号正文可选择“微信原生增强、极简墨白、深蓝科技”三套原创预置主题。导出菜单提供使用同一主题定义和行内编译器的手机预览，320px、375px、414px 三档宽度会自动并行检查横向溢出、局部滚动、过小文字、文字对比度和标题间距，用户可切换查看每档明细。主题只改变排版表现，不修改 Markdown、图片/公式/Mermaid 替换顺序或任务完成状态；最终主题 ID 会写入人工验收记录。本地宽度审计不会自动确认真实公众号移动端验收。
...
当前源码基线：42 个测试文件、224 项测试通过；严格类型检查和生产构建通过。P1 可访问性、错误恢复与工作区体验已加入读屏状态播报、标准文档标签语义、键盘分栏、解析重试、标签循环切换/关闭/排序、最近文件安全入口和解析性能观测。公众号主题预览采用真正的模态焦点闭环。真实 Electron UI 冒烟已验证快捷键、性能指标和最近文件 IPC 不暴露路径。打包后的发布门禁还会逐项运行基础启动、Chromium PDF、DOCX Utility Process、离线 HTML Utility Process、KaTeX 公式 PNG、Mermaid PNG 和完整 UI 共 7 组生产态冒烟。
...
当前最新开发产物为 `release/fantastic-editor-0.3.0-rc.1-portable.exe`，并同时生成 `release/fantastic-editor-0.3.0-rc.1-setup.exe`；隐私门禁、便携版隔离启动、七组生产态冒烟以及安装—启动—卸载门禁均已通过，两个产物均为 `NotSigned`，不能作为签名发行版。RC 发布清单记录了安装版 SHA-256 `CCD3212F6E8C45D85A7F3BF66EA056C814530AB117C62B547D0A1ABA2A7FFAB1` 和便携版 SHA-256 `1ED7305A921BA9A84E4B1BA99B73B4A1B0C6974D0F61F68C12D415A5242BC05B`。签名流水线已经就绪；本机尚无有效代码签名证书，未来取得受信任证书后再执行 `npm run dist:signed`。历史 `0.2.0-rc.*` 产物继续保留，不被覆盖。

公众号逐主题真实验收状态仍以已归档记录为准：微信原生增强在 `rc.2` 下通过，深蓝科技有 `rc.1` 历史完整记录；`rc.3` 没有新增真实平台记录，因此当前候选不能标记为公众号三主题兼容性正式验收完成。
